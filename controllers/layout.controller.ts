...
export const createLayout = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if(exist){
        return next(new Errorhandler(`${req.body.type} Layout already exist`,400))
      }
      ...
      if(type==='Category'){
         ...
         await LayoutModel.create({type:'Category',categories:categoryItem});
         res.status(201).json({ 
            success: true,
...
export const EditLayout=CatchAsyncErrors(async(req:Request, res:Response,next:NextFunction)=>{
    try {
        const {type}=req.body
         ...
         if(type==='Banner'){
          ...
          if(banner){
            ...
            await LayoutModel.findOneAndUpdate({type:"Banner"},newBanner,{new:true})
            res.status(201).json({
                success:true,
        ...
        //    await category.save()
           ...
           await LayoutModel.findByIdAndUpdate(category._id,{type:"Category",categories:categoryItem})
